...
```{r}
my_mat <- t(my_mat) 
my_mat #matrix was flipped along the diagonal axis from top left to bottom right
```
...
##### e. For the first variable, write a single line of R code to identify which rows have the missing values. Re-order the entire data frame to arrange the second variable in alphabetical order.
```{r}

```

##### f. Calculate the column mean for the first variable. Rename the second column ‘my_LETTERS’
```{r}
